...
async function convertFunfileToDockerfile(
  funfilePath,
  dockerfilePath,
  runtime,
  serviceName,
  ...
  const dockerfileContent = await parser.funfileToDockerfile(
    funfilePath,
    runtime,
    serviceName,
    functionName,
...
export async function processFunfileForBuildkit(
  ...
  if (enableBuildkitServer) {
    execSync(
      `buildctl --addr tcp://${buildkitServerAddr}:${buildkitServerPort} build --no-cache --frontend dockerfile.v0 --local context=${baseDir} --local dockerfile=${path.dirname(
        ...
        dockerfilePath,
      )} --output type=local,dest=${baseDir}`,
      {
        stdio: 'inherit',
      },
    );
